<template>
    <div class="row__livescore-detail" :class="{'fade-in':$store.state.livescoreSelected.match_code==''}">
        ...
        <livescoredetail :items="$store.state.dataLivescoreDetail" :class="{'livescore-detail--shrink':$store.state.livescoreSelected.isopening==true}">
            <template slot-scope="{items}">
                <livecastlivescore :items="items"></livecastlivescore>
            </template>
        </livescoredetail>
    </div>
</template>
